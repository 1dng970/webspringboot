...
public class cdn_controller {
	...
	public String mysql_list() throws Exception{
        ...
        rs.next();
		this.log.info(rs.getString("nidx"));		
	...
	*/
	...
	/*
	 ...
	 해당 버그를 수정하여 FTP 파일이 없어도 DB가 삭제되도록 수정 	  
	 */
	
	//사용자가 해당파일을 클릭 시 파일을 자신의 PC ,Mobile에 다운로 드 할 수 있는 API 메소드 
	@GetMapping("/cdn/download.do/{filenm}")
	public void downloads(@PathVariable(name="filenm") String filenm,
			HttpServletResponse res) throws Exception {
		//외부에서 이미지 및 파일, 동영상을 FTP URL 로 읽어서 byte로 변환시 무조건 File 객체 이용 
		/*ftp://
		File f = new File(filenm); //FTP 경로를 로드 한 후 byte 변환
		byte [] files = FileUtils.readFileToByteArray(f);
		*/
	  URL url = new URL("http://kbsn.or.kr/su/"+filenm);
	  //http 통신
	  HttpURLConnection httpcon = (HttpURLConnection)url.openConnection();
	  //해당 경로에 이미지를 byte로 읽어들임
	...
	//defaultValue = "" 안넣으면 null값뜸 , 꼭 넣어주기 
