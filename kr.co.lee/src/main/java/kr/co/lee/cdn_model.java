...
public class cdn_model {
	...
	//CDN 서버에 있는 해당 파일을 삭제하는 역할 model
	public boolean cdn_ftpdel(String newfiles,String idx) throws Exception{
		...
		//PASSIVEMODE : 가상의 PORT를 이용하여 연결을 설정
		...
		if(this.fc.login(this.user, this.pass)) {
		 //FTP 서버에서 파일 삭제 
	     //this.result =   this.fc.deleteFile("/su/"+newfiles);//파일경로 에 있는 사진 삭제
		//this.result =   this.fc.removeDirectory("/su");
			this.result =   this.fc.makeDirectory("/su");//디렉토리 생성
	     if(this.result ==true) { //정상적으로 FTP 파일을 삭제 한 후 
	    	 this.cs.cdn_delete(idx); //고유값으로 데이터를 DB 내용을 삭제 
	...
	public boolean cdn_ftp(MultipartFile files,String url) throws Exception {
		try {
			...
			if(this.fc.login(this.user, this.pass)) {	//FTP 로그인
				 ...
				 this.result = this.fc.storeFile("/su/"+new_file, files.getInputStream());
