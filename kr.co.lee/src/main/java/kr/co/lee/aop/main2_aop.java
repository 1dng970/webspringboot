...
@Aspect //AOP
@Component //main2_aop가 무조건 실행 되도록 함
public class main2_aop {
	
	Logger log = LoggerFactory.getLogger(this.getClass()); 

	
	//Controller에서 setter 값을 입력받은 사항을 AOP getter로 받아서 추가코드를 작성하게 됨 
	...
	//@After() : 해당 Controller로 작동 된 후 실행되는 AOP
	//@Before : 해당 Controller가 작동 되기 전 실행되는 AOP
	
	
	//로그아웃되고 나서 언제 로그아웃 했는지 DB를 insert 하도록 코드를 AOP로 작성하시오
	...
	public void testaop_a() throws Throwable { //Throwable : 상위 예외처리
	   try {		   
	...
	} catch (Throwable e) { //Exception 보다 상위 Throwable 예외처리
		e.printStackTrace();
	}
	}
}
